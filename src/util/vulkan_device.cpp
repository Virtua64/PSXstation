                                            ...
                                            bool enable_debug_utils)
{
  ...
  if (wi.type == WindowInfo::Type::SDL)
  {
    ...
    if (!SDL_Vulkan_GetInstanceExtensions(static_cast<SDL_Window*>(wi.window_handle), &sdl_extension_count, nullptr))
    {
      ERROR_LOG("SDL_Vulkan_GetInstanceExtensions() failed: {}", SDL_GetError());
      return false;
    }

    std::unique_ptr<const char*[]> sdl_extensions = std::make_unique<const char*[]>(sdl_extension_count);
    if (!SDL_Vulkan_GetInstanceExtensions(static_cast<SDL_Window*>(wi.window_handle), &sdl_extension_count,
                                          sdl_extensions.get()))
    {
      ERROR_LOG("SDL_Vulkan_GetInstanceExtensions() failed (2): {}", SDL_GetError());
      return false;
    }
