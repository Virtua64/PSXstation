...
{
  rc_client_t* client;
  rc_client_fetch_all_progress_callback_t callback;
  void* callback_userdata;
  uint32_t console_id;
  rc_client_async_handle_t async_handle;
...
static void rc_client_fetch_all_progress_callback(const rc_api_server_response_t* server_response, void* callback_data)
{
  ...
  error_message =
    rc_client_server_error_message(&result, server_response->http_status_code, &ap_response.response);
  if (error_message)
  {
    RC_CLIENT_LOG_ERR_FORMATTED(client, "Fetch all progress for console %u failed: %s",
                                ap_callback_data->console_id, error_message);
    ap_callback_data->callback(result, error_message, NULL, client, ap_callback_data->callback_userdata);
  }
  else
  {
    rc_client_all_progress_t* list;
    const size_t list_size = sizeof(*list) + sizeof(rc_client_all_progress_entry_t) * ap_response.num_entries;

    list = (rc_client_all_progress_t*)malloc(list_size);
    if (!list)
    {
    ...
    {
      rc_client_all_progress_entry_t* entry = list->entries =
        (rc_client_all_progress_entry_t*)((uint8_t*)list + sizeof(*list));
...
rc_client_async_handle_t* rc_client_begin_fetch_all_progress(rc_client_t* client, uint32_t console_id,
                                                             rc_client_fetch_all_progress_callback_t callback,
                                                             void* callback_userdata)
{
  ...
  {
    callback(RC_INVALID_STATE, "client must be logged in or logging in", NULL, client, callback_userdata);
    return NULL;
  }
...
void rc_client_destroy_all_progress(rc_client_all_progress_t* list)
{
  free(list);
}
